...
def update_D_P1_o2(H, J, m, m_p, C_p):
    D = np.einsum('i,ij,jl->il', 1 - m**2, J, C_p, optimize=True)
    D += np.einsum('i,ij, il ,jl,l->il', 2 * m * (1 - m**2), J, J, C_p, m_p, optimize=True)
    return D
...
def update_C_P2_o1(H, J, m, m_p, C_p):
    ...
    inv_D= 1/D
    inv_D[D==0] = 0
    rho = np.einsum( 'i,k,ij,kl,jl->ik', np.sqrt(inv_D), np.sqrt(inv_D), J,J, C_p, optimize=True)
    rho = np.clip(rho, -1, 1)
    for i in range(size):
        C[i, i] = 1 - m[i]**2
...
def update_P1D_o2(H, J, m_p, C_p,D_p):
    ...
    V_ik -= np.einsum('ii,kl,il->ik', J, J, C_p, optimize=True) + np.einsum('ij,kk,jk->ik', J, J, C_p, optimize=True)
