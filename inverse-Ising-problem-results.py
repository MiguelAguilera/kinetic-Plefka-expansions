...
for ind in range(len(betas)):
	...
	H=data['H']
	J=data['J']
	errorH[:,ind]=np.array([np.mean((H-HP_t1_t)**2), np.mean((H-HP_t)**2), np.mean((H-HP_t1)**2), np.mean((H-HP2_t)**2)])
	errorJ[:,ind]=np.array([np.mean((J.flatten()-JP_t1_t.flatten())**2), np.mean((J.flatten()-JP_t.flatten())**2), np.mean((J.flatten()-JP_t1.flatten())**2), np.mean((J.flatten()-JP2_t.flatten())**2)]  )
...
ax[0, 1].plot(J.flatten(),JP_t1_t.flatten(), 'v', color=colors[0], ms=5, rasterized=True)
ax[0, 1].plot(J.flatten(),JP_t.flatten(), 's', color=colors[1], ms=5, rasterized=True)
ax[0, 1].plot(J.flatten(),JP_t1.flatten(), 'd', color=colors[2], ms=5, rasterized=True)
ax[0, 1].plot(J.flatten(),JP2_t.flatten(), 'o', color=colors[3], ms=5, rasterized=True)
